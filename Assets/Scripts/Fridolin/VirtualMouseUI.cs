using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.InputSystem.LowLevel;
using UnityEngine.InputSystem.UI;

public class VirtualMouseUI : MonoBehaviour
{
    // see this link for reference
    // https://www.youtube.com/watch?v=j2XyzSAD4VU

    [SerializeField] public RectTransform canvasRectTransform;
    [SerializeField] private RectTransform mouseVisualRectTransform;
    private VirtualMouseInput virtualMouseInput;


    private void Awake()
    {
        virtualMouseInput = GetComponent<VirtualMouseInput>();

        // Auto-assign canvas if not set
        if (canvasRectTransform == null)
        {
            Canvas canvas = FindObjectOfType<Canvas>();
            if (canvas != null)
            {
                canvasRectTransform = canvas.GetComponent<RectTransform>();
            }
            else
            {
                Debug.LogError("Canvas not found in scene for VirtualMouseUI.");
            }
        }
    }

    private void Update()
    {
        transform.localScale = Vector3.one * (1f / canvasRectTransform.localScale.x);
        transform.SetAsLastSibling();
        mouseVisualRectTransform.transform.localScale = Vector3.one * ( canvasRectTransform.localScale.x);
    }

    private void LateUpdate()
    {
        Vector2 virtualMousePosition = virtualMouseInput.virtualMouse.position.value;
        virtualMousePosition.x = Mathf.Clamp(virtualMousePosition.x, 0f, Screen.width);
        virtualMousePosition.y = Mathf.Clamp(virtualMousePosition.y, 0f, Screen.height);
        InputState.Change(virtualMouseInput.virtualMouse.position, virtualMousePosition);
        Debug.Log(virtualMouseInput.virtualMouse.delta.ReadValue());
        Debug.Log("virtualMousePosition:" + virtualMousePosition);

    }
}
